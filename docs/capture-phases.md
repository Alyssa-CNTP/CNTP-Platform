...
| **1B** Serialization | **Built and provisioned; one env var from live** | Code wired in all four output sections (Sieving, Refining, Granule, Blender — Pasteuriser is out of the scheme by design, §5). **The migration IS applied on staging** — verified read-only 2026-09-04: `production.bag_serial_counters` returns `42501 permission denied`, which only an EXISTING table raises; a missing one gives `42P01`. So the only thing left is `NEXT_PUBLIC_FF_DB_SERIAL_ALLOCATION` in staging's env. Until then the race is still live. **2026-09-15:** the Blender was approved for rollout, so the value should include `blender,smallblender` (alongside `sieving`) when it is set — until it is, the Blender keeps minting legacy serials like `25SGNAT26C-1-1-20`, which carry no work centre and no date and are why Blender bags do not read like every other section's in Bag Tracking. |
...
| **4a** Stoppage ledger | **Built; both migrations applied** | Done ahead of Phase 4 proper because it was a live data-loss bug, not a refactor — see below. `production.timesheet_stoppages` is append-only with per-row upsert and void-not-delete, i.e. the Phase 4 pattern, applied to timesheets rather than to bags. `20260909_002_timesheet_stoppages.sql` and `20260909_004_stoppage_coverage_and_supervisor_call.sql` are both applied to **staging and production** — verified 2026-09-10 by probing each project for `production.timesheet_stoppages`, `v_machine_downtime`, `v_line_downtime` and `supervisor_requested_at`. Order still matters if either is ever re-run elsewhere: 002 first, because 004 needs its table. |
| **4** Ledger foundation | **Partly started, out of order** | `production.bag_lineage` (2026-09-15) is the Phase 4 pattern — append-only, per-row diff insert, void-not-delete — applied to bag *parentage* rather than to bag events, because the genealogy chain in Bag Tracking could not populate at all without it. Still outstanding: `lib/core/ledger/` absent; `scan_events` unextended; `live/capture/page.tsx` still bulk-deletes the ledger. **Migration `20260915_003` is NOT applied** — see below. |
