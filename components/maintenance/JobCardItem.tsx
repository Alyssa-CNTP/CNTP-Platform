...
// Subtle two-state toggle: tinted-active vs quiet-inactive — no saturated blocks.
...
// Quiet section container — hairline border, soft tint, generous padding.
  ...
  return (
    <div className="rounded-xl border border-surface-rule bg-surface-card shadow-sm p-4 mb-3.5">
      ...
      <div className="text-[13px] text-text-muted mt-1.5">
        ...
        {j.assigned_to ? <>{' · '}{j.external ? 'External' : 'Tech'} {j.assigned_to}</> : null}
      </div>
      <div className="text-[13px] text-text mt-1">{j.description}</div>

      {/* Secondary detail collapsed behind a quiet toggle */}
      {hasDetail && (
        <>
