...
export function LiveCaptureKPIs() {
  ...
  const flags    = rows.filter(r => r.kgIn > 0 && Math.abs(r.variance) > massBalanceToleranceKg(r.kgIn)).length
          ...
          <table className="w-full">
            ...
            <tbody className="divide-y divide-surface-rule">
              {rows.map(r => {
                ...
                const flag = r.kgIn > 0 && Math.abs(r.variance) > massBalanceToleranceKg(r.kgIn)
