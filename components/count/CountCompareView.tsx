...
// ── COMPONENT ──────────────────────────────────────────────────────────────────
export default function CountCompareView({ sessionId, date, onClose }: Props) {
  const db = getDb()
    ...
    <div className="space-y-4">
      ...
      <div className="flex items-start justify-between gap-4">
        ...
        {onClose && (
          <button
            onClick={onClose}
            className="flex-shrink-0 text-sm text-text-muted hover:text-text transition-colors"
          >
            Close
          </button>
        )}
              ...
              <div key={secId} className="card overflow-hidden">
                ...
                >
                  <div className="flex-1 text-left">
                    ...
                    {varCount > 0 && (
                      <span className="ml-2 font-mono text-[10px] bg-warn-bg text-status-warn border border-warn/30 px-2 py-0.5 rounded-full">
                        {varCount} variance{varCount !== 1 ? 's' : ''}
                      </span>
                    )}
                  </div>
                  <span className="font-mono text-[11px] text-text-muted">{secLines.length} items</span>
                  {isOpen
