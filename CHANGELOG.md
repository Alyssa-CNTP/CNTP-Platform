...
- **Its own layout, not the production bag label**, because it carries what the QC measured: product, grade/variant, Code 128 of the bag serial, and Bulk Density / Leaf Shade / PA Level / Residue, with Lot, Product and Date in the footer — the product is written out again next to the lot number so the label reads correctly without relying on the heading. Out-of-spec in-process results and a failed Final QC print as a black warning band.
