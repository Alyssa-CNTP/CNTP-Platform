...
## 2026-08-25 — Alyssa (Pasteuriser: final-product output tags — per-box tags, pallet tags, and a pallet barcode that resolves its contents)

**Files changed:** `supabase/migrations/20260821_002_pasteuriser_pallets.sql` (new), `lib/production/pallet.ts` (new), `lib/production/label-final.ts` (new), `lib/production/label-final-pplb.ts` (new), `components/production/capture/PalletPanel.tsx` (new), `app/api/print/final-label/route.ts` (new), `app/api/production/pallet/[serial]/route.ts` (new), `components/production/capture/PasteuriserCapture.tsx`, `app/(app)/production/capture/[section]/page.tsx`

The Pasteuriser packs the final product, so its output tags are where the whole chain back to the Sieving Tower has to be readable. Everything that leaves the line — every box, every paper bag, and every pallet — now carries its own barcode, and one pallet scan lists the boxes on it.

- **Fixed: the "print label" button on a bagging line produced an unscannable tag.** It printed `line.serial` (e.g. `PR-190826-001`), a line identifier, while `persist()` writes the real per-box rows as `{LOT}-{NNN}`. That serial has no `bag_tags` row, so the printed tag came back *not found* at every downstream scan. Line-level tagging is removed; tags are now issued per box and per pallet, off the same serial the database actually stores (`makeBoxSerial`, shared by the print path and the write path so they cannot drift).
- **Fixed: the final-product tag hardcoded `grade: 'A'`.** Confirmed with the floor that final product carries **no grade at all** — grade (A Export / B Export Blend / C Domestic) is a raw-material property that stops at the blender, and a finished blend mixes grades by design (job card 26249 is 50% A + 45% B + 5% granules), so any single letter is false. The new tag shows the **variant only** (CON / ORG / RA CON / RA ORG). Also confirmed the `-C` in `30FPSFC-KUN25-C` is *Conventional*, not grade C, so grade is not derived from the item code either.
- **`production.pallets` + `bag_tags.pallet_id` / `bag_tags.bag_number`.** A pallet is a real parent row with its own serial, tag and scan history — not a prefix baked into the box serial, which would make the pallet un-scannable on its own and force a re-palletised box to have its primary key rewritten, breaking every `scan_event` already logged against it. `pallet_id` is `ON DELETE SET NULL`, so dropping a mis-keyed grouping never deletes the physical boxes with it. `bag_number` stores the bag number off PR-FM-005 ("Starting/Ending Bag Number") so a tag can read "BAG 281 OF 315" without string-parsing a serial.
- **Pallets are proposed, then confirmed by the operator** (`PalletPanel`). Boxes-per-pallet comes from the packaging spec (45 for boxes, 18 for paper bags, 1 for a 500kg bulk bag) and is editable per line, because the last pallet of a batch is nearly always short and a paper-bag order palletises differently from a box order. Nothing is written or printed until the operator commits. Re-splitting after a bag-count correction preserves an already-printed record **only where the pallet still covers exactly the same bags** — matching on position alone would claim tags exist for boxes that were never tagged.
- **Two independent tag actions per pallet — the pallet's own tag, and the box tags on it** — because they happen at different moments: boxes are tagged as they come off the line, the pallet once it's built. Each can be printed or recorded as hand-written, which the paper→system transition still needs.
- **A 315-box batch is one print job, not 315.** `/api/print/final-label` takes an array and concatenates the PPLB blocks into a single socket write (browsers block repeated popups, and no operator can babysit 315 print dialogues). The fallback to browser print **reports which path was used** rather than falling back silently — an operator who thinks 45 box tags went to the Argox will otherwise walk to the printer and find nothing.
- **`GET /api/production/pallet/[serial]`** — the "one barcode that counts the entire pallet" read path, backed by `production.v_pallet_contents`. It returns the pallet's *declared* contents alongside the boxes *actually* linked to it, deliberately: a pallet declared at 45 boxes with only 43 linked is a real discrepancy, and one merged number would hide it.
- **New label template, kept separate from the signed-off in-process bag tag** (`label-print.ts` / `label-pplb.ts` are untouched). Carries the Cape Natural logo, a `BOX`/`PALLET` marker so the two can't be confused at a glance, and a double border on pallet tags. Verified by rendering: the batch/lot was being ellipsised to `26249-CON-...` on an even four-column footer — the one field every downstream lookup keys on — so the columns are now weighted. Dates are formatted `DD/MM/YYYY` in SAST via `formatToParts`, because `toLocaleDateString('en-ZA')` resolves to `YYYY/MM/DD` in Chromium and the floor reads the tag beside a form that uses `19-08-2026`.
- **Scope note:** this is the CNTP *internal traceability* tag, and it does not attempt destination-country retail labelling rules — those govern the human-readable content and live on the customer's own label, specified per job card by its "Bag markings" field. The two coexist on the box.
- **Not done, deliberately:** the logo is browser-print only. Putting a bitmap on the Argox needs a GW/GM graphics download to printer memory, and that printer has already been observed silently dropping a downloaded custom font — an unproven download risks tags that print blank on the floor. Thermal tags carry the full text layout without the logo.
...
Reported: operators sometimes weigh and print a bag's label before it's physically full (e.g. 127kg at end of shift), then a later shift adds material to the *same* physical bag rather than starting a new one — today that means hand-writing a corrected weight next to the printed label while `bag_tags` silently stays wrong, or a duplicate serial gets minted for the same bag. Fix: one serial per physical bag, forever, with full traceability of every top-up.

- New `bag_tags.is_open` boolean (not a new `status` value — `status='in_stock'` already drives several "available to consume downstream" pickers, so a separate flag avoids touching their semantics). All 4 of those pickers now exclude open bags — a still-filling bag can't be pulled into the next section. `qms.v_pending_bag_qc` also excludes them (via a join to `bag_tags`, not by touching `qms.v_bag_events`/`v_bag_qc_status` — those were substantially rewritten since this was first drafted, most recently a LATERAL-subquery rewrite that fixed a production query timing out at ~20s, and CREATE OR REPLACE VIEW only allows new columns at the very end of a view's own output, so adding `is_open` there would have required reinserting a column in the middle of `v_bag_qc_status` — Postgres rejects that).
- `scan_events` gains a `'topped_up'` action. Every in-scope capture component already logs a `'bagging_out'` event at creation, so this makes the ledger complete: a top-up's `weight_kg` holds the delta added (same convention as the existing `'debagging_in'` rows), not a running total.
...
Reported: the Records/Analytics list felt like too much to take in at a glance, hard to tell which line a row belonged to, the balance-flag warning icon showed on nearly every row regardless of whether it meant anything, and the header felt cramped.

Rebuilt on a new shared kit (`components/production/ui/kit.tsx`) — `Panel`/`PanelHead` (no drop-shadows, hairline borders, no tinted header bars), `Stat`/`StatRow` with sparklines, `ShareBar`/`BarRow` for part-to-whole and magnitude instead of table walls, `Collapse` to hide exact numbers behind a disclosure (still available, printable), and `SectionChip` as one consistent section-identity marker used everywhere. Colour discipline follows a validated, colourblind-safe luminance ramp — magnitude is one hue, part-to-whole uses the ordered ramp with labels, identity comes from the chip/axis label, never colour alone.

- **Balance flag now only reads as a warning when it actually is one** — the pill always shows the kg figure, but the warning triangle and amber tone only appear when a record is genuinely outside tolerance.
- **Section identity is a consistent coloured chip** on every row and in the per-line analytics table, not just a small colour-coded initial.
- **Analytics' "Per line" table is genuinely per-line** (aggregated per section), kept separate from the Records view's per-record list — the two were reading as one confusing mix before.
- **Mobile/tablet**: a compact kg-in → kg-out → yield line replaces the desktop mini-bar chart below `sm`, so the figures that matter are still visible on a phone without the chart needing to shrink into unreadability.
- Same filters, same API routes, same edit/reopen/archive actions as before — this is a visual/structural rebuild on the same data, not a behaviour change.

This was a substantially complete, uncommitted redesign found sitting in the working tree from a prior session — reviewed in full (compiles clean, no TODOs, no unrelated dependencies) before shipping rather than rebuilt from scratch.
...
Built the Pasteuriser section's live tablet capture — the final-product line — modelled on the proven Blender/Refining components and plugged into the existing shared shell (`[section]/page.tsx`), so it inherits production runs, cross-shift carry-over (07h00→01h00), the 16h00 PIN changeover, checks/cleaning/overview/sign-off, autosave and offline recovery with no new plumbing. Additive only — no change to the Sieving/Refining/Granule/Blender capture components.

- **New `PasteuriserCapture.tsx`.** Debagging consumes Blender output bags across two streams — main (**D**) and post-sieve blending (**E**) — via scan / pick-from-system / manual-register, the same three-mode pattern as Refining/Blender. Because the pasteuriser deliberately consumes *finished blend* product, its scan lookup passes `blockFinishedProducts: false` (the one setting that would otherwise reject every legitimate blend bag). Bagging captures final-product pallet lines (**A**): kind (Final Product / High Moisture / Refill), item + Acumatica code, lot, bag-number range and kg/bag — total auto-calculated. Also by-products (**B** — Brown Dust / First Cut / Alt / Extraction Dust / Purge), packaging reconciliation (boxes/foils/quad-seal, qty, damaged), product-label summary (received/discarded/handed-over), scale verification (std vs actual), floor waste (**C**), and the paper's mass balance `(A+B+C) − (D+E)`.
- **Output codes prefill from the Job Card, with a Master-Inventory override.** A picker over `job_cards_pasteuriser` prefills batch no, blend code, final-product item + `30FP…` code, weight/bag and packaging; an `ItemPicker` over `production.inventory_items` lets the operator confirm/correct the exact item. Per-bag **Print label / Write on tag** choice (prints via the existing pipeline to the pasteuriser's Argox printer).
- **Smart, non-blocking traceability.** `PasteuriserCapture` flags — but never blocks — the two mistakes the paper is prone to: bagging final product before anything was debagged, and an output batch number that wasn't debagged this session (the classic typing error). Centralised in one place so it can be tightened to a hard-block later.
- **Central lot/batch capitalisation.** New `lib/production/normalize-code.ts` `upperCode()`, applied at the one write-layer chokepoint in `persist()` to every stored `lot_number` (debagging + bagging, all sections) so "gs-0271" and "GS-0271" can never read as two different lots in traceability checks. Only `lot_number` is normalised — never `bag_serial_no`, which is a `bag_tags` FK whose exact case must be preserved.
- **Shell wiring.** `[section]/page.tsx`: registered the pasteuriser in the production factory, component switch, `prodTotals`, run discriminator (keyed on the final-product batch number for carry-over), `hasCaptureData`, and `buildDebag`/`buildBag` (writes `prod_debagging`/`prod_bagging`). `CaptureOverview.tsx`: added a pasteuriser discriminator so its debag/output shapes aren't misread as Sieving. `capture-config.ts`: marked pasteuriser `built`.

Verified via full-repo `tsc --noEmit` (clean for all touched files) and a dev-server compile of `/production/capture/pasteuriser` (200, no console errors). Not yet exercised end-to-end on the floor — needs a rostered pasteuriser assignment and operator login. No migration required (rich state lives in `prod_sessions.draft_data`; `bag_tags`/`prod_debagging`/`prod_bagging` already permit `section_id = 'pasteuriser'`).

**Deferred to follow-up (agreed this session):** barcode label redesign (Type/Variant layout across HTML/ZPL/PPLB) and the phone-scanner camera fix (`BagScanner.tsx` null-video-ref race + swallowed `play()` + iOS `BarcodeDetector` fallback).
...
## 2026-07-22 — Alyssa (Shift Roster: new periods now pre-fill, silent write failures surfaced; removed Shuaib from live roster)

**Files changed:** `app/(app)/production/roster/page.tsx`

Fixes the report that "auto-rotate / pre-fill worked for my user but not for others, and a new period comes up blank." Root cause was not permissions/RLS — on staging any authenticated user can write the roster (anon is blocked, `authenticated` is open). The real gaps were in the client:

- **"New period" now pre-fills from the current roster.** `createPeriod` previously created a completely blank period for everyone — only "Generate next week" carried people over (with rotation). Supervisors who used "New period" got an empty sheet. `NewPeriodModal` now has a "Pre-fill from the current roster" checkbox (default on when a roster is loaded) that copies all current people into the new period as-is (same shifts, no rotation); unchecking starts blank.
- **Write failures are no longer silent.** `createPeriod` and `generateNextPeriod` did not check insert errors — a blocked/failed `roster_entries` insert left an empty period with no feedback, which read as "it didn't work / didn't show." Added a dismissible error banner and error checks on every period/entry write, with actionable messages.
- **De-duplicated rotation logic.** `generateNextPeriod` now uses the shared `rotateEntries` helper (`lib/production/roster-rotate.ts`) — the same logic the unattended Sunday cron uses — so the button and the cron can't diverge. Added `PERIOD_SELECT`/`ENTRY_SELECT` column constants.
- **Data (staging):** removed Shuaib Sentso (maintenance_manager) from the current (20–24 Jul) and upcoming (27–31 Jul) roster periods — 2 entries deleted. Two past-period entries (6–10 Jul, 13–17 Jul) left intact as history. **Production DB not touched from this session — Shuaib likely needs the same removal on prod.**

---

## 2026-07-22 — Alyssa (Production data consolidation: canonical batch spine + yield/batch analytics report)

**Files changed:** `supabase/migrations/20260721_002_batch_spine.sql` (new), `supabase/migrations/20260721_003_yield_views.sql` (new), `supabase/migrations/20260721_004_order_reconciliation.sql` (new), `lib/production/batch-key.ts` (new), `app/(app)/production/capture/[section]/page.tsx`, `app/api/production/yield-analytics/route.ts` (new), `app/api/production/batch/[key]/route.ts` (new), `app/api/production/reconciliation/route.ts` (new), `components/production/YieldAnalytics.tsx` (new), `components/production/BatchConsolidation.tsx` (new), `app/(app)/production/analytics/page.tsx` (new), `app/(app)/traceability/page.tsx`, `app/(app)/tags/page.tsx`, `app/(app)/production/orders/page.tsx`, `components/production/ProductionTabs.tsx`, `components/layout/Sidebar.tsx`, `app/(app)/layout.tsx`, `lib/auth/permission-registry.ts`

Consolidation foundation so capture, bags, quality and (next) Acumatica orders join on one identity, plus the first interactive yield report on top. Requires the two new migrations to be applied to the **staging** Supabase project via the SQL editor before the report shows data (run each migration's PART 1 then PART 2 separately).

- **Phase 1 — canonical batch spine.** Added `production.normalize_batch(text)` (single source of truth for the canonical form of a lot/batch string; collapses `"GS - 0098"` → `"GS-0098"`) and a `production.batches` dimension keyed on the normalized `batch_key`, backfilled from existing lots. Added a nullable `batch_id` FK to `prod_sessions`, `bag_tags`, `prod_debagging`, `prod_bagging`, `production_runs` (backfilled by match). Mirrored the normalizer in TS (`lib/production/batch-key.ts`) and wired the capture `persist()` to upsert batches and stamp `batch_id` on the session, its input/output rows and the run on every save. Fixes the root cause of cross-domain joins being fragile string-equality across four schemas.
- **Phase 2 — reporting query layer.** Added owner-privileged views in the `production` schema (previously there were none): `v_session_yield`, `v_output_stream` (per-product output share — the Fine Leaf/total ratio), `v_machine_params` (pivots the checks engine's VSD / indent speed·angle / screen config per session), `v_batch_quality` (normalize-joins `qms.sd_runs` + `qms.quality_records` — bulk density, leaf shade, PA, residue), and `v_batch_360` (consolidated per batch). Quality values surfaced as latest-value text (qms column types unverified); granule moisture left as a TODO pending qms schema confirmation.
- **Phase 4 — interactive report.** New `/production/analytics` page + `/api/production/yield-analytics` route reading the views: KPI row, output mix, daily yield trend (70% target line), yield by section/variant, machine-setting-vs-yield scatter, a batch table linking output to quality, and a data-completeness banner. Added an "Analytics" tab to the Production hub, a sidebar entry, a route guard (`Production`/`Management`), and a `production.analytics` permission resource.
