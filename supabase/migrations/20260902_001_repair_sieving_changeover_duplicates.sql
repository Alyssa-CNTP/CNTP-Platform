...
-- sibling batch's row as "missing" and adopted it, and persist() — which writes
-- the whole session back from draft_data — made the copies permanent. Every
-- page load doubled them: 8 → 16 → 32 → 64 → 128 → 258 rows for 41 physical
...
-- serial-less twin — the `—` rows on the production order, and the unidentified
-- bags clogging Quality's awaiting-QC queue.
--
-- The code fault is fixed separately (lib/production/self-heal-reconcile.ts +
...
-- 4 change data, and step 4 must not be skipped. Step 6 verifies. Run one step
...
-- `debag_rows` vs `distinct_bags` is the duplication factor. A healthy session
-- has them equal. Rows with no bag label are never counted as duplicates —
-- two different unlabelled bags are two bags, not one.
with sieving as (
  select id, date, shift from production.prod_sessions where section_id = 'sieving'
),
d as (
  select s.id as session_id, s.date, s.shift,
         count(*) filter (where pd.product_type in ('Farm Bag', '500kg Farm Bag') and pd.is_spillage = false) as debag_rows,
         count(distinct case
           when pd.product_type in ('Farm Bag', '500kg Farm Bag') and pd.is_spillage = false
                and coalesce(btrim(pd.notes), '') <> ''
           then upper(btrim(coalesce(pd.lot_number, ''))) || '|' || btrim(pd.notes)
         end) as distinct_labelled_bags,
         count(*) filter (where pd.product_type in ('Farm Bag', '500kg Farm Bag') and pd.is_spillage = false
                            and coalesce(btrim(pd.notes), '') = '') as unlabelled_rows
  from sieving s
...
select d.date, d.shift, d.session_id,
       d.debag_rows, d.distinct_labelled_bags + d.unlabelled_rows as debag_rows_expected,
...
from d left join b on b.session_id = d.session_id
...
-- ── Step 2 · READ ONLY · Exactly which rows Step 3 and Step 4 will remove ───
-- Read this before running anything that writes. `keep_or_drop = 'DROP'` rows
...
with ranked as (
  select pd.id, pd.session_id, pd.bag_no, pd.lot_number, pd.notes, pd.kg_nett,
         pd.created_at,
         row_number() over (
           partition by pd.session_id, upper(btrim(coalesce(pd.lot_number, ''))), btrim(pd.notes)
...
-- 2b · prod_bagging — a serial-less row that matches a serialed bag on
--      session + product + weight + bagging time is that bag's nulled twin.
--      A genuine serial-less by-product row will not coincide on all four.
with spine as (
...
-- 2c · SAFETY CHECK — this must return NO ROWS before Step 3 is run.
--      Deduplication keys on (session, lot, bag label) because a farm bag is a
--      physical object debagged once. If two rows share that identity but
--      disagree on weight they are NOT copies of one another, and dropping one
--      would lose a real figure. Confirmed empty for 31-08-2026 against the
--      floor's paper sheet (41 bags, 41 distinct pairs) — check it again here
...
declare
  ...
  v_unsafe int;
begin
  -- Refuse to run at all if Step 2c is not clean. An identity whose rows
  -- disagree on weight is not a set of copies, and dropping one would lose a
  -- real figure — so nothing is deleted until that is resolved by hand.
  ...
  if v_unsafe > 0 then
    raise exception
      'Aborted: % bag identit(ies) have rows that disagree on weight — they are not copies. Run Step 2c and resolve them by hand first.', v_unsafe;
  end if;
  ...
  with ranked as (
    select pd.id,
           row_number() over (
             partition by pd.session_id, upper(btrim(coalesce(pd.lot_number, ''))), btrim(pd.notes)
             order by pd.bagging_time nulls last, pd.created_at, pd.bag_no, pd.id
           ) as rn
    from production.prod_debagging pd
    join production.prod_sessions s on s.id = pd.session_id and s.section_id = 'sieving'
    where pd.product_type in ('Farm Bag', '500kg Farm Bag')
  ...
  saved as (
    ...
    select 'prod_debagging', pd.session_id, pd.id, to_jsonb(pd)
    from production.prod_debagging pd join doomed d on d.id = pd.id
    returning 1
  )
  delete from production.prod_debagging pd using doomed d where pd.id = d.id;
  ...
  saved as (
    insert into production.repair_20260902_backup (source, session_id, row_id, payload)
    select 'prod_bagging', pb.session_id, pb.id, to_jsonb(pb)
    from production.prod_bagging pb join doomed d on d.id = pb.id
    returning 1
  )
  delete from production.prod_bagging pb using doomed d where pb.id = d.id;
...
declare
  ...
  k           text;
begin
  if jsonb_typeof(prods) <> 'array' then return dd; end if;

  -- seen_* span EVERY batch of the session, because the copies live in the
  -- other batches, not alongside the original in its own.
  for p in select value from jsonb_array_elements(prods) loop
    ...
    for r in select value from jsonb_array_elements(coalesce(p -> 'data' -> 'debag', '[]'::jsonb)) loop
      ...
      k := upper(btrim(coalesce(nullif(r ->> 'lot', ''), p ->> 'lot', ''))) || '|' || btrim(coalesce(r ->> 'bag_no', ''));
...
-- 4a · READ ONLY preview — how many rows each session loses.
select s.id, s.date, s.shift,
       (select count(*) from jsonb_array_elements(s.draft_data -> 'productions') pr,
                             jsonb_array_elements(coalesce(pr.value -> 'data' -> 'debag', '[]'::jsonb))) as debag_before,
       (select count(*) from jsonb_array_elements(production.dedupe_sieving_draft(s.draft_data) -> 'productions') pr,
                             jsonb_array_elements(coalesce(pr.value -> 'data' -> 'debag', '[]'::jsonb))) as debag_after,
       (select count(*) from jsonb_array_elements(s.draft_data -> 'productions') pr,
                             jsonb_array_elements(coalesce(pr.value -> 'data' -> 'outputs', '[]'::jsonb))) as outputs_before,
       (select count(*) from jsonb_array_elements(production.dedupe_sieving_draft(s.draft_data) -> 'productions') pr,
                             jsonb_array_elements(coalesce(pr.value -> 'data' -> 'outputs', '[]'::jsonb))) as outputs_after
...
-- 4b · WRITES — back up the document, then rewrite it.
with changed as (
...
-- exactly one row per physical bag, and it — not draft_data — is the source of
-- truth on the output side. Voiding tags on the strength of draft_data would
...
-- 6a · Every session should now show debag_rows = debag_rows_expected and no
--      unexplained serial-less bagging rows. Re-run Step 1 — it should return
--      no rows at all.

-- 6b · What was removed:
--   select source, count(*), min(captured_at)
--   from production.repair_20260902_backup group by source;

-- 6c · Put a deleted debagging row back:
--   insert into production.prod_debagging
--   select * from jsonb_populate_record(null::production.prod_debagging, payload)
--   from production.repair_20260902_backup where id = <backup id>;
--
--   …the same shape works for prod_bagging. To restore a session's draft_data:
--   update production.prod_sessions s set draft_data = b.payload
--   from production.repair_20260902_backup b
--   where b.source = 'prod_sessions.draft_data' and b.session_id = s.id and b.id = <backup id>;
