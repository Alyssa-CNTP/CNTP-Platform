...
// A production manager sends a generated (or manually-typed) Granule job card
// to the supervisor(s) for approval — this IS their "Verify & Sign": the
...
// card, never trusted from the client. Mirrors the Pasteuriser job card's
// send-for-approval route exactly, targeting job_cards_granule instead.

export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  const { id: cardId } = await params
  const caller = await getCallerPermissions()
  if (!caller.userId) return NextResponse.json({ error: 'Not authenticated' }, { status: 401 })
  if (!caller.can('can_generate_job_cards_granule')) return NextResponse.json({ error: 'Permission denied' }, { status: 403 })
  ...
  try {
    ...
    await notify({
      ...
      title: `Job card ready for approval — ${card.item_no || card.product_name || 'Granule'}`,
      body: `${managerName} generated a Granule job card${card.batch_number ? ` (batch ${card.batch_number})` : ''} and needs your approval.`,
