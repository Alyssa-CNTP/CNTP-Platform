...
// A production supervisor approves or rejects a Granule job card a manager
// sent for approval. Approving IS the supervisor's "Verify & Sign" — their
// signature is resolved server-side from production.employee_signatures
// (their own Staff Directory record), never accepted from the client.
// Mirrors the Pasteuriser job card's decide route exactly, targeting
// job_cards_granule instead.

export async function PATCH(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  const { id: cardId } = await params
  const caller = await getCallerPermissions()
  if (!caller.userId) return NextResponse.json({ error: 'Not authenticated' }, { status: 401 })
  if (!caller.can('can_approve_job_cards_granule')) return NextResponse.json({ error: 'Permission denied' }, { status: 403 })
  ...
  const { data: card, error: cErr } = await admin.from('job_cards_granule')
  ...
  const patch: any = decision === 'approved'
    ? { status: 'approved', approved_by: caller.userId, approved_at: now, rejected_reason: null,
        ...
        batch_id: await resolveBatchId(card.batch_number, 'granule') }
    : { status: 'rejected', rejected_reason: reason }

  const { data: updated, error: uErr } = await admin.from('job_cards_granule')
  ...
  try {
    if (card.created_by) {
      ...
      const label = card.item_no || card.product_name || 'Granule'
      await notify({
        recipients, kind: 'job_card_decision', source: 'production',
        refTable: 'job_cards_granule', refId: String(cardId),
