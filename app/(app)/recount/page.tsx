...
function VarianceItem({
  ...
  async function submit() {
    if (!recountKg || !notes.trim()) {
      setError('Recount weight and notes are required.')
      return
    }
    setSub(true)
    setError(null)
    ...
    <div className="bg-surface-card border border-surface-rule rounded-2xl overflow-hidden">
      ...
      >
        ...
        <div className="text-right shrink-0">
          <div className="font-display font-bold text-[15px] text-warn">
            {entry.variance_kg.toFixed(1)} kg off
          </div>
        ...
        <div className="border-t border-surface-rule px-5 pb-5 space-y-4 pt-4">
          ...
          <div className="space-y-1.5">
            ...
            <textarea
              ...
              placeholder="e.g. Found 2 extra bags behind the pallet. Admin count appears to have missed them. Recount confirms 1,252 kg."
              rows={3}
...
function RecountCard({ req }: { req: RecountRequest }) {
  ...
  return (
    <div className={`bg-surface-card border rounded-2xl p-4 space-y-2 ${
      ...
      {req.notes && (
        <p className="font-body text-[12px] text-text-muted italic">"{req.notes}"</p>
      )}
      {req.admin_notes && (
...
export default function RecountPage() {
  ...
  async function load(isRefresh = false) {
    ...
    if (sess?.id) {
      // Get variance entries from the view
      ...
      // Get existing recount requests for this session
      ...
      setExisting(((reqs as any[]) ?? []).map((r: any) => ({
        ...r,
  ...
  // Codes already submitted
  const submittedCodes = new Set(existing.map(r => r.inventory_code))

  // Only show variances not yet submitted
  ...
  return (
    <div className="px-4 py-5 space-y-5 max-w-[720px]">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="font-display font-bold text-[22px] text-text">Recount</h1>
      ...
      {!session && (
        <div className="bg-surface-card border border-surface-rule rounded-2xl p-8 text-center space-y-2">
          ...
          <p className="font-body text-[13px] text-text-muted">
            No completed count session for today yet.
          </p>
      ...
      {/* Session exists but no variances */}
