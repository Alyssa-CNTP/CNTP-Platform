...
import { takeinDb, loadDepots, visibleDepots, allocateDocNo, logBatchEvent, releaseBatchNo } from '@/lib/takein/db'
...
interface Row {
  ...
  panel_outcome: string | null; panel_grade: string | null; panel_reason: string | null
...
export default function IntakePage() {
  ...
  const load = useCallback(async () => {
    ...
    } catch (e: any) { setErr(e?.message ?? 'Could not load deliveries.') }
  ...
  async function issueGrn() {
    ...
    try {
      ...
      await load()
    } catch (e: any) { setErr(e?.message ?? 'Could not make out the GRN.') }
  ...
  async function signGrn(name: string) {
    ...
    try {
      ...
      await load()
    } catch (e: any) { setErr(e?.message ?? 'Could not record the signature.') }
    finally { setBusy(false) }
  }
  ...
  async function confirmDialog(category: string, reason: string) {
    ...
    try {
      ...
      setDialog(null)
      await load()
    } catch (e: any) { setErr(e?.message ?? 'Could not complete that.') }
    finally { setBusy(false) }
  }
