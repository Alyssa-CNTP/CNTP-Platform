...
import { takeinDb, loadDepots, visibleDepots, logBatchEvent, releaseBatchNo } from '@/lib/takein/db'
...
export default function MiniLabPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const { data: sh } = await getDb().schema('qms' as any)
    ...
    } catch (e: any) { setErr(e?.message ?? 'Could not load the mini lab.') }
  ...
  const mini  = b?.lab_results?.find((l: any) => l.source === 'mini') ?? null
  ...
  const queue = useMemo(() => rows.filter(r => {
    ...
    if (!g) return false
    const lab = (s: string) => r.lab_results?.find((l: any) => l.source === s) ?? null
    const st = stageOf({ mini: lab('mini'), internal: lab('internal'), external: lab('external'),
                         ...
                         panel: r.panel_outcome ? { outcome: r.panel_outcome as any, grade: r.panel_grade,
  ...
  const gradingInput = b ? {
    mini, internal: b.lab_results?.find((l: any) => l.source === 'internal') ?? null,
    external: b.lab_results?.find((l: any) => l.source === 'external') ?? null,
    organic: /organic/i.test(b.contract?.variant ?? ''), binding: {},
  } : null

  async function saveMini(fields: Record<string, unknown>) {
    if (!b || !canCapture || locked) return
    const next = { batch_id: b.id, source: 'mini',
                   ...(mini ?? {}), ...fields,
                   captured_by: user?.id ?? null, captured_by_name: actor.name }
    delete (next as any).source_row
    const { error } = await takeinDb().from('lab_results')
      .upsert(next, { onConflict: 'batch_id,source' })
    if (error) { setErr(error.message); return }
    setRows(rs => rs.map(r => r.id !== b.id ? r : {
      ...r, lab_results: [...(r.lab_results ?? []).filter((l: any) => l.source !== 'mini'), next],
    }))
  }
  ...
  async function doReturn(category: string, reason: string) {
    ...
    try {
      ...
      setReturning(false)
      await load()
    } catch (e: any) { setErr(e?.message ?? 'Could not return the load.') }
              ...
              <li key={r.id}>
                <button onClick={() => setSelId(r.id)}
                  ...
                  <div className="font-mono text-[10px] text-text-faint">
                    {r.lab_results?.some((l: any) => l.source === 'mini') ? 'in progress' : 'not started'}
                  </div>
                </button>
              </li>
            ))}
