...
import { takeinDb, loadDepots, visibleDepots, deliveryDepots, allocateBatchNo, logBatchEvent } from '@/lib/takein/db'
...
export default function SchedulePage() {
  ...
  const load = useCallback(async () => {
    ...
    } catch (e: any) { setErr(e?.message ?? 'Could not load the calendar.') }
  ...
  async function createBooking(form: {
    ...
    try {
      ...
      setSlot(null)
      await load()
    } catch (e: any) { setErr(e?.message ?? 'Could not save the booking.') }
  ...
  async function openDelivery(b: Booking) {
    ...
    try {
      ...
      if (error) throw error
      const batchId = (data as any).id as string
    ...
    } catch (e: any) { setErr(e?.message ?? 'Could not open the delivery.') }
    finally { setBusy(false) }
  }
      ...
      <section className="rounded-2xl border border-surface-rule bg-surface-card">
        ...
        <div className="px-4 py-3">
          ...
          <div className="overflow-x-auto">
            <table className="w-full min-w-[46rem] table-fixed border-separate border-spacing-[3px]">
              <thead>
                <tr>
                  <th className="w-14" />
                  {days.map(ds => (
