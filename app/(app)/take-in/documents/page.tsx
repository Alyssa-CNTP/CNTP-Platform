...
export default function DocumentsPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      for (const r of (t as PanelTermRow[] | null) ?? []) (map[r.contract_id] ??= {})[r.term_key] = r.binding
      setTerms(map)
  ...
  const lab = (s: string) => b?.lab_results?.find((l: LabRow) => l.source === s) ?? null
  const gradingInput = b ? {
    ...
    panel: b.panel_outcome ? { outcome: b.panel_outcome, grade: b.panel_grade,
  ...
  async function issueAfl() {
    ...
    try {
      ...
      await logBatchEvent(b.id, 'afleweringsbewys_issued',
        ...
        + `${frozen?.pct.toFixed(2)} % sieving`, actor, { ...frozen })
      await load()
    } catch (e: unknown) { setErr(errMsg(e, 'Could not make out the Afleweringsbewys.')) }
  ...
  async function issueCoa() {
    ...
    try {
      ...
      await load()
    } catch (e: unknown) { setErr(errMsg(e, 'Could not make out the COA.')) }
    finally { setBusy(false) }
  }
  ...
  async function doWithdraw(category: string, reason: string) {
    ...
    try {
      ...
      setWithdraw(null)
      await load()
    } catch (e: unknown) { setErr(errMsg(e, 'Could not withdraw the document.')) }
    finally { setBusy(false) }
  }
