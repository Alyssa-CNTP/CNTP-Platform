...
import type { Depot } from '@/lib/takein/types'
...
interface Row {
  ...
  lab_results: { source: string; sieve_json: any; moisture: number | null
                 density: number | null; shade: number | null; aroma: number | null
                 colour: number | null; taste: number | null; agrees: boolean | null
                 residue_group: string | null; pa_group: string | null }[]
...
export default function TakeInOverviewPage() {
  ...
  async function load() {
    ...
    try {
      ...
      setErr(e?.message ?? 'Could not load take-in data.')
  ...
  const graded = useMemo(() => rows.map(r => {
    ...
    const st = stageOf({
      ...
      panel: r.panel_outcome
        ? { outcome: r.panel_outcome as any, grade: r.panel_grade,
