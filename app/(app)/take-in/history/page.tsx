...
import { takeinDb, loadDepots, visibleDepots } from '@/lib/takein/db'
import { stageOf, isFinalised, nettKg, type Stage } from '@/lib/core/takein/grading'
import type { Depot, BatchEvent } from '@/lib/takein/types'
import { Loader2, AlertTriangle, Search } from 'lucide-react'

interface Doc {
  id: string; kind: string; doc_no: string; issued_at: string; issued_by_name: string | null
  signed_by: string | null; signed_at: string | null; frozen: any
...
export default function HistoryPage() {
  ...
  const [show, setShow]     = useState<'open' | 'done' | 'void' | 'all'>('open')
  ...
  const load = useCallback(async () => {
    ...
    } catch (e: any) { setErr(e?.message ?? 'Could not load history.') }
      ...
      <section className="rounded-2xl border border-surface-rule bg-surface-card px-4 py-4">
        <div className="grid gap-3 sm:grid-cols-[1fr_260px]">
          ...
          <label className="block">
            ...
            <select value={show} onChange={e => { setShow(e.target.value as any); setSelId(null) }}
