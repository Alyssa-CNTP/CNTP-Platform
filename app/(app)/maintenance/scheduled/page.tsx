  ...
  return (
    <div className="p-4 sm:p-6 max-w-[1400px] mx-auto space-y-6">
      <div>
        ...
        <p className="text-sm text-text-muted mt-1">Weekly and monthly checklists per area, plus annual calibration and verification due dates.</p>
      </div>
                          ...
                          return (
                            <div key={ti} className="mb-2">
                              <div className="flex items-center gap-2">
                                ...
                                <span className={`text-[12px] ${s.done ? 'text-text-muted line-through' : 'text-text'}`}>{task}</span>
                              </div>
              ...
              <table className="data-table">
                ...
                <tbody>{annualRows.map(a => (
                  <tr key={a.id}>
                    ...
                    <td>{a.supplier !== 'Internal' && <button className="border border-surface-rule bg-surface-card text-text rounded-md px-2.5 py-1.5 text-[11px] font-semibold hover:border-text/25 transition" onClick={() => setPopup('Draft Email to ' + a.supplier + ':\n\nSubject: ' + a.category + ' Due — ' + a.asset + '\n\nDear ' + a.supplier + ',\n\nPlease schedule ' + a.category.toLowerCase() + ' for:\nAsset: ' + a.asset + '\nSerial: ' + a.serial_no + '\nDue: ' + fmtD(a.next_due) + '\n\nPlease confirm.\n\nRegards,\nCNTP Maintenance')}>Email</button>}</td>
