...
export default function ProductionOrderDetailPage() {
  ...
  const { section_id, date, status, shifts, bags, bagsOutputKg, rebagRows, freshTopUps, debags, massBalance: mb, timesheets, takeovers } = day
      ...
      <Panel>
        ...
        <PanelBody>
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
            <Field label="Date" value={format(new Date(date), 'd MMM yyyy')} />
            <Field label="Shifts" value={shifts.map(s => SHIFT_LABEL[s.session.shift] ?? s.session.shift).join(' + ')} />
            <Field label="Operators" value={operators.join(', ') || '—'} />
