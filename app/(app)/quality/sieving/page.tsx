...
export default function SievingPage() {
  ...
  // qms.bag_qc_waivers (20260902_003 here; 20260915_003 recreates it
  // idempotently). A waived bag drops out of v_pending_bag_qc, so without this
  // the queue would simply be shorter and nothing on screen would say a bag had
  // been closed unsampled — the number gets better and the reason disappears.
  // Shown with who accepted it, because a waiver is a Quality decision on
  // record: it is not a pass, and must never read as one.
  ...
  return (
    <div className="p-5 max-w-[1400px]">
      ...
      {/* Also renders for the three non-actionable groups — part bags still
          filling, bags closed without sampling, and rows with no serial — so
          none of them is merely absent from the queue with nothing on screen
          saying why. */}
      {(bagAlerts.length > 0 || unidentifiedBagCount > 0 || waivedCount > 0 || partBags.length > 0) && (
        <div style={{position:'fixed',top:70,right:16,zIndex:5000,display:'flex',flexDirection:'column',gap:8,maxWidth:340,maxHeight:'min(60vh, calc(100vh - 90px))',pointerEvents:'none'}}>
          <button onClick={()=>setAlertsCollapsed(c=>!c)}
            ...
            <span>📦 {bagAlerts.length} bag{bagAlerts.length>1?'s':''} awaiting QC{partBags.length > 0 ? ` · ${partBags.length} still filling` : ''}{unidentifiedBagCount > 0 ? ` · ${unidentifiedBagCount} unidentified hidden` : ''}{waivedCount > 0 ? ` · ${waivedCount} closed unsampled` : ''}</span>
