...
export type PermissionKey =
  ...
  | 'can_read_logistics'
  // No can_read_notebooks: production has no Note Books module. It is a
  // staging-only feature — no /notebooks pages, no can_access_notebooks key,
  // no row in PERMISSION_MATRIX — so a grant key for it would be an orphan the
  // drift test rejects. It arrives when the module does.
  | 'can_read_management'
  | 'can_read_workspace'
  | 'can_read_staff'
  | 'can_read_training'
...
export const ALL_PERMISSION_KEYS: PermissionKey[] = [
  ...
  'can_read_marketing','can_read_bag_tracking','can_read_logistics',
