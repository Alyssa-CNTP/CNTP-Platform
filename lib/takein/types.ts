...
export interface Batch {
  ...
  panel_outcome:  PanelOutcome | null
...
/** The panel's verdict. Named so the screens can narrow a database string to it
 *  once, instead of each casting its own way past the compiler. */
export type PanelOutcome = 'accept' | 'downgrade' | 'reject'

/** A `lab_results` row as the capture screens select it — every field the
 *  grading core reads (LabResultLike + ExternalLike) plus the ones the screens
 *  show. Declared once here because eight screens embed it in their own row
 *  type, and eight copies is how two of them end up disagreeing. */
